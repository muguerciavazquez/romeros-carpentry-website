# ROMERO'S CARPENTRY Website

Single-page website concept for a Jacksonville-area residential remodeling company.

Client slogan: Built Right. Built To Last.

## Current Status

- Main page: `index.html`
- Built as a premium black-and-gold remodeling website for local SEO and lead conversion.
- Sections included: hero, services, before-and-after comparison, service areas, project gallery, FAQ, reviews, and quote request.
- Customer help features included: floating chat assistant example and Google Calendar scheduling call to action.
- Previous logo files are still in `assets/` for reference only.
- The full ROMERO'S CARPENTRY logo is stored in `assets/romeros-carpentry-logo.png`.
- The header uses the cropped symbol logo at `assets/romeros-carpentry-symbol.png`.
- Temporary generated visuals are stored in `assets/` until the client provides real project photos.
- SEO support files included: `robots.txt`, `sitemap.xml`, and `site.webmanifest`.

## Next Build Priorities

1. Connect the quote form to a CRM or form service if email-only delivery is not enough.
2. Replace temporary generated visuals with real ROMERO'S CARPENTRY project photos.
3. Expand service-area blocks into dedicated SEO landing pages.
4. Add verified Google review content after client approval.
5. Replace the logo image with a transparent SVG or PNG when available for the cleanest production header.
6. Replace the Google Calendar placeholder URL with the client's real appointment schedule link.

## Trustbyte Notes

The site should focus on conversion for homeowners in Jacksonville, Ponte Vedra, Nocatee, St. Augustine, and nearby areas. The strongest proof will come from before-and-after remodeling photography, clear service categories, and a direct free-estimate call to action.

